...
import {KalturaWebexAPIUsersMatching} from "kaltura-ngx-client/lib/api/types/KalturaWebexAPIUsersMatching";

@Component({
    selector: 'kWebexEditProfile',
    templateUrl: './edit-profile.component.html',
    styleUrls: ['./edit-profile.component.scss'],
    providers: [CategoriesSearchService, KalturaLogger.createLogger('EditWebexProfileComponent')]
})
export class EditWebexProfileComponent implements OnInit, OnDestroy {
    @Input() parentPopup: PopupWidgetComponent;
    @Input() profile: KalturaWebexAPIIntegrationSetting | null;

    @Output() onSave = new EventEmitter<KalturaWebexAPIIntegrationSetting>();
    ...
    ngOnInit() {
        if (this.profile) {
            ...
            this._showTranscription = typeof this.profile.enableTranscription !== "undefined";
    ...
    private _setInitialValue(profile: KalturaWebexAPIIntegrationSetting): void {
        let optInGroupNames = [];
        ...
        this._profileForm.setValue({
            ...
            description: profile.webexAccountDescription || '',
            deleteContent: profile.deletionPolicy === KalturaNullableBoolean.trueValue,
            transcription: profile.enableTranscription === KalturaNullableBoolean.trueValue,
            createUser: profile.createUserIfNotExist === KalturaNullableBoolean.trueValue,
            postfix: profile.userMatchingMode,
            userPostfix: profile.userPostfix,
            participation: profile.handleParticipantsMode,
            upload: profile.groupParticipationType || 0,
            categories: profile.webexCategory ? [{name: profile.webexCategory}] : [],
    ...
    public _save(): void {
        ...
        this.profile.webexAccountDescription = formValue.description;
        ...
        if (this._showTranscription) {
            this.profile.enableTranscription = formValue.transcription ? KalturaNullableBoolean.trueValue : KalturaNullableBoolean.falseValue;
        }
        if (this._enableMeetingUpload) {
        ...
        this.profile.userMatchingMode = formValue.postfix;
        this.profile.userPostfix = formValue.userPostfix;
    ...
    private searchCategories(text: string) {
        return Observable.create(
            observer => {
                ...
                return () => {
                    console.log("webex profile edit: searchCategories(): cancelled");
