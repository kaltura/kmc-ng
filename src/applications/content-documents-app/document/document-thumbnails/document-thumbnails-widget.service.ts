...
import { ContentDocumentViewSections } from "app-shared/kmc-shared/kmc-views/details-views";
...
@Injectable()
export class DocumentThumbnailsWidget extends DocumentWidget {
    ...
    constructor(private _kalturaServerClient: KalturaClient, private _appAuthentication: AppAuthentication,
        ...
        super(ContentDocumentViewSections.Thumbnails, logger);
    }
    ...
    protected onActivate(firstTimeActivating: boolean) {
        ...
        const canLoadProfiles = false; // this._permissionsService.hasPermission(KMCPermissions.CONTENTDISTRIBUTION_PLUGIN_PERMISSION);
    ...
    public _setAsDefault(thumb: ThumbnailRow): void {
        ...
        const documentId = this.data ? this.data.id : null;
        ...
        this._kalturaServerClient.request(new ThumbAssetSetAsDefaultAction({thumbAssetId: thumb.id}))
            ...
            .subscribe(
                () => {
                    if (documentId) {
                        this._appEvents.publish(new PreviewMetadataChangedEvent(documentId));
                    }
