...
import {KalturaClient, KalturaDocumentEntry} from 'kaltura-ngx-client';
...
import { NewDocumentRelatedFile } from './new-document-related-file';
...
@Injectable()
export class DocumentRelatedWidget extends DocumentWidget implements OnDestroy
{
  ...
  private _trackUploadFiles(): void {
    this._uploadManagement.onTrackedFileChanged$
      ...
      .pipe(filter(uploadedFile => uploadedFile.data instanceof NewDocumentRelatedFile))
      .pipe(map(uploadedFile => {
        let relevantRelatedFile = null;
        if (uploadedFile.data instanceof NewDocumentRelatedFile) {
        ...
        ({ relevantRelatedFile, uploadedFile }) => {
          switch (uploadedFile.status) {
            ...
            case TrackedFileStatuses.prepared:
              relevantRelatedFile.serverUploadToken = (<NewDocumentRelatedFile>uploadedFile.data).serverUploadToken;
              this._syncBusyState();
              break;
              ...
              .setCompletion(response => {
                if (response.error) {
                  ...
                  if (relevantUploadFile) {
                    ...
                    (<NewDocumentRelatedFile>relevantUploadFile.data).assetId = response.result.id;
                  }
                }
              });
  ...
  private _updateAssetsResponse(response: KalturaAttachmentAssetListResponse): void {
    response.objects.forEach((asset: RelatedFile) => {
      ...
      const relevantFile = this._uploadManagement.getTrackedFiles().find(file =>
        file.data instanceof NewDocumentRelatedFile && file.data.assetId === asset.id
      );
      ...
      if (relevantFile) {
        ...
        asset.serverUploadToken = (<NewDocumentRelatedFile>relevantFile.data).serverUploadToken;
      }
    });
  }
  ...
  private _validateFileSize(file: File): boolean {
    ...
    return this._uploadManagement.supportChunkUpload(new NewDocumentRelatedFile(null)) || fileSize < maxFileSize;
  }
  ...
  public _onFileSelected(selectedFiles: FileList) {
    if (selectedFiles && selectedFiles.length) {
      ...
      const entryRelatedFiles = files.map(file => new NewDocumentRelatedFile(file));
