...
import { DocumentMetadataWidget } from './document-metadata-widget.service';
...
@Component({
    selector: 'kDocumentMetadata',
    templateUrl: './document-metadata.component.html',
    styleUrls: ['./document-metadata.component.scss']
})
export class DocumentMetadataComponent implements AfterViewInit, OnInit, OnDestroy {
    ...
    constructor(public _widgetService: DocumentMetadataWidget,
    ...
    _searchTags(event) : void {
        ...
        this._searchTagsSubscription = this._widgetService.searchTags(event.query).subscribe(data => {
                const suggestions = [];
                const documentTags = this._widgetService.metadataForm.value.tags || [];

                (data|| []).forEach(suggestedTag => {
                    const isSelectable = !documentTags.find(tag => {
    ...
    _searchCategories(event) : void {
        ...
        this._searchCategoriesSubscription = this._widgetService.searchCategories(event.query).subscribe(data => {
                const suggestions = [];
                const documentCategories = this._widgetService.metadataForm.value.categories || [];


                (data|| []).forEach(suggestedCategory => {
                    const label = suggestedCategory.fullName + (suggestedCategory.referenceId ? ` (${suggestedCategory.referenceId})` : '');

                    const isSelectable = !documentCategories.find(category => {
    ...
    _updateDocumentCategories($event : any) : void{
