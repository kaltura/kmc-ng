      ...
      .subscribe(
        response => {
          if(response.objects.length) {
            let _flags = DropFolderListType.ADD_NEW | DropFolderListType.MATCH_OR_KEEP | DropFolderListType.MATCH_OR_NEW | DropFolderListType.XML_FOLDER,
                df:KalturaDropFolder;
            ...
            response.objects.forEach(object => {
              if(object instanceof KalturaDropFolder) {
                ...
                if(df.fileHandlerType === KalturaDropFolderFileHandlerType.content) {
                  ...
                  else if (_flags & DropFolderListType.MATCH_OR_KEEP && cfg.contentMatchPolicy === KalturaDropFolderContentFileHandlerMatchPolicy.matchExistingOrKeepInFolder) {
                    this.ar.push(df);
                  }
                  else if (_flags & DropFolderListType.MATCH_OR_NEW && cfg.contentMatchPolicy === KalturaDropFolderContentFileHandlerMatchPolicy.matchExistingOrAddAsNew) {
                    this.ar.push(df);
                  }
